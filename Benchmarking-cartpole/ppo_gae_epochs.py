...
def policy_loss(old_log_prob, log_prob, advantage, eps, done_index):
    ...
    done_mask = torch.cat([torch.zeros_like(ratio[:done_index]), torch.ones_like(ratio[done_index:])]).int()
    terminal_mask = torch.cat([torch.ones_like(ratio[:-1]), torch.zeros_like(ratio[-1:])]).int()
    # ratio = ratio * done_mask
    # ratio = ratio + done_mask.bitwise_not()
    ratio = ratio * terminal_mask
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(int(args.total_timesteps))):
        ...
        for epoch in range(args.update_epochs):
            ...
            dist = torch.distributions.Categorical(probs=probs)
            new_actions = actions.cpu().numpy().tolist()
            new_actions.pop(0)
            new_actions.insert(len(new_actions), 0.)
            new_logprobs = dist.log_prob(torch.tensor(new_actions, dtype=torch.float32, device=device))
