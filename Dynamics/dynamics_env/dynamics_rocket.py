...
class Dynamics(gym.Env):
    def __init__(self, hyperparameters):
        ...
        self.high = np.array([self.box_scale * 1.0, self.box_scale * 1.0, self.box_scale * 1.0, 1.0, 1.0, 1.0, self.rocket_mass * 1.0]) # pos x, y, z, vel x, y, z, m
        self.low = np.array([-self.box_scale * 1.0, -self.box_scale * 1.0, -self.box_scale * 1.0, -1.0, -1.0, -1.0, 0.0])
        self.action_bounds = np.array([self.max_thrust * 1.0, self.max_thrust * 1.0, self.max_thrust * 1.0, 1.0]) # thrust~mdot*ve x, y, z, on/off
