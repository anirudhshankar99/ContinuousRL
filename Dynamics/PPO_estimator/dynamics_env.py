...
class Dynamics(gym.Env):
    ...
    def reset_2(self):
        ...
        ax, ay, az = self.get_acceleration(self.phase_coords[:-3])
        state = np.concat([self.phase_coords, ax, ay, az])
        return self._normalise_state(state), self._get_info_2()
    ...
    def step_2(self, action):
        ...
        ax, ay, az = self.get_acceleration(self.phase_coords[:-3])
        state = np.concat([self.phase_coords, ax, ay, az])
        return self._normalise_state(state), reward, done, False, self._get_info_2()
    ...
    def _process_actions(self, action):
        return np.clip(action, self.low[:-3] / 10, self.high[:-3] / 10)

    def _process_actions_2(self, action):
        return action #np.clip(action / self.high[:-3], self.low[:-3], self.high[:-3])
