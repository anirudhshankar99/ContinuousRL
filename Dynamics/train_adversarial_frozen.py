...
def parse_args():
    ...
    parser.add_argument('--pretrain-weights-dir', type=str, default='Dynamics/runs/Dynamics-v0__pre'+os.path.basename(__file__).rstrip("_frozen.py")+'_pretrain_predictor.pth')
...
if __name__ == '__main__':
    ...
    agent_predictor.load_state_dict(torch.load(args.pretrain_weights_dir))
    if args.log_train:
    ...
    with tqdm(range(int(args.total_timesteps)), desc=f'episodic_reward: {episodic_reward}') as progress:
        for i in range(int(args.total_timesteps)):
            ...
            state_list.append([episodic_reward]+actions_proposer[0].tolist())
            progress.set_description(f'episodic_reward: {episodic_reward}')
            progress.update()
    state_list = np.array(state_list)
    save_mask = state_list[:,0] > ((np.max(state_list[:,0]) - np.min(state_list[:,0])) / 2 + np.min(state_list[:,0]))
    columns = ['reward']
