...
class MLP(torch.nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dim=64):
        ...
        self.network = torch.nn.Sequential(
            torch.nn.Linear(input_dim, hidden_dim),  # Input: (x, y, vx, vy)
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_dim, hidden_dim),
            torch.nn.ReLU(),
            torch.nn.Linear(hidden_dim, output_dim)  # Output: (Δx, Δy, Δvx, Δvy)
        )
...
class GaussianMLP(torch.nn.Module):
    ...
    def forward(self, x, y):
        x = self.model(x)
        means, stds = torch.split(x, [self.output_dim, self.output_dim], dim=-1)
        pdf = self.pdf(means, stds, y)
        return pdf
    ...
    def pdf(self, means, stds, x):
        return 1/(torch.sqrt(2*torch.pi*stds**2))*torch.exp((-(x - means)**2)/(2*stds**2))
...
if __name__ == '__main__':
    ...
    mlp = MLP(env.observation_space.shape[0] + env.action_space.shape[0], env.observation_space.shape[0]).to(device)
    Gmlp = GaussianMLP(env.observation_space.shape[0] + env.action_space.shape[0], env.observation_space.shape[0]).to(device)
    adam_mlp = torch.optim.Adam(mlp.parameters(), lr=1e-5)
    adam_gmlp = torch.optim.Adam(Gmlp.parameters(), lr=1e-5)
    ...
    with tqdm(range(int(args.total_timesteps)), desc=f'episodic_reward: {episodic_reward}') as progress:
        for i in range(int(args.total_timesteps)):
            ...
            while j < args.num_steps:
                ...
                mlp_input = torch.cat((old_state, action))
                mlp_output = mlp(mlp_input)
                Gmlp_output = Gmlp(mlp_input, state)
                mlp_loss = mlp_criterion(mlp_output, state)
                adam_mlp.zero_grad()
                mlp_loss.backward()
                adam_mlp.step()
                squashed_pdf = sigmoid(Gmlp_output)
                gmlp_loss = mlp_criterion(squashed_pdf, torch.ones_like(squashed_pdf))
                with torch.no_grad():
                    means, stds = Gmlp.get_prediction(mlp_input)
                adam_gmlp.zero_grad()
                gmlp_loss.backward()
                adam_gmlp.step()
                mlp_losses.append(mlp_loss.detach().cpu().numpy())
                gmlp_losses.append(mlp_criterion(means, state).cpu().numpy())
                gmlp_stds.append(stds.cpu().numpy())
            ...
            if args.log_train:
                ...
                writer.add_scalar("loss/mlp", np.mean(mlp_losses), global_step=i)
                writer.add_scalar("loss/gmlp", np.mean(gmlp_losses), global_step=i)
                writer.add_scalar("loss/gstds", np.mean(gmlp_stds), global_step=i)
