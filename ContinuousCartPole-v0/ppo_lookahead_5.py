...
class DynamicsModel(torch.nn.Module):
    ...
    def select_action(self):
        ...
        highest_probability, most_probable_action = torch.max(probabilities[:-1]), torch.argmax(probabilities[:-1])
        return (most_probable_action, 0) if highest_probability > 1/args.lookahead_options else (np.random.randint(low=0, high=args.lookahead_options), 1) # return the most probable action if clear majority (>50%) else return a random index
