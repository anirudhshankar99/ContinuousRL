...
class MLP(nn.Module):
    def __init__(self, env):
        ...
        self.model = nn.Sequential(
            nn.Linear(np.array(env.observation_space.shape).prod() + env.action_space.shape[0], 64),
            nn.Tanh(),
            nn.Linear(64, 32),
            nn.Tanh(),
            nn.Linear(32, np.array(env.observation_space.shape).prod())
        )
        self.trajectories = []
...
class Memory():
    ...
    def add(self, observations, actions, rtgs):
        ...
        # forget
        forget_mask = torch.ones_like(self.rtgs) * (torch.rand(self.rtgs.shape) > args.forget_frac)
        self.rtgs *= forget_mask
        self.rtgs, sorted_indices = torch.sort(self.rtgs, descending=True)
        self.source_state_action = self.source_state_action[sorted_indices]
        self.dest_state = self.dest_state[sorted_indices]

        # if len(self.dest_state) + len(rtgs) - 1 > args.memory_size:
...
def parse_args():
    ...
    parser.add_argument('--forget-frac', type=float, default=0.01,
                        help='fraction of memories forgotten every episode')
...
if __name__ == '__main__':
    ...
    update = 0
    episodic_reward = 0
    trajectory_sampling_dist = torch.distributions.Normal(0, args.lookahead_options // 10)
    with tqdm(range(int(args.total_timesteps)), desc=f'episodic_reward: {episodic_reward}') as progress:
        for i in range(int(args.total_timesteps)):
            ...
            while j < args.num_steps:
                ...
                selected_trajectory_index = torch.floor(torch.abs(trajectory_sampling_dist.sample())) // 2
                best_trajectory = sorted_trajectories[int(selected_trajectory_index.item())]
