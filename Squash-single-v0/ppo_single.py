...
class MLP(nn.Module):
    def __init__(self, env, activation=nn.ReLU):
        super().__init__()
        ...
        self.scaling = torch.tensor(env.observation_space.high - env.observation_space.low, dtype=torch.float32, device=device)
        self.disp = torch.tensor(env.observation_space.low, dtype=torch.float32, device=device)

        self.action_scaling = torch.tensor(env.action_space.high - env.action_space.low, dtype=torch.float32, device=device)
        self.action_disp = torch.tensor(env.action_space.low, dtype=torch.float32, device=device)
    
    def forward(self, x):
        return self.model(x)
    
    def normalize_state(self, x):
        return (x - self.disp) / self.scaling
    
    def normalize_action(self, x):
        return (x - self.action_disp) / self.action_scaling
...
def parse_args():
    ...
    parser.add_argument('--gym-id', type=str, default='Squash-single-v0',
    ...
    parser.add_argument('--ball-start', type=str, default='center',
                        help='where the ball starts from at reset.(), \'center\' or \'random\'')
...
if __name__ == '__main__':
    ...
    mlp = MLP(env).to(device)

    adam_actor = torch.optim.Adam(actor.parameters(), lr=3e-4)
    adam_critic = torch.optim.Adam(critic.parameters(), lr=1e-3)
    adam_mlp = torch.optim.Adam(mlp.parameters(), lr=3e-4)
    ...
    with tqdm(range(int(args.total_timesteps)), desc=f'episodic_reward: {episodic_reward}') as progress:
        for i in range(int(args.total_timesteps)):
            ...
            while j < args.num_steps:
                ...
                except ValueError: print(i,j,action_means, action_stds, state) 
                action = dist.sample()
                ...
                predicted_state = mlp(torch.cat([mlp.normalize_state(state), mlp.normalize_action(action)]))
                ...
                mlp_loss = torch.nn.functional.mse_loss(predicted_state, mlp.normalize_state(state))
                adam_mlp.zero_grad()
                mlp_loss.backward()
                adam_mlp.step()
            ...
            if args.log_train:
                ...
                writer.add_scalar("loss/mlp_loss", np.mean(mlp_losses), global_step=i)
